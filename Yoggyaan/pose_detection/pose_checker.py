...
def _target_score(actual: float, ideal: float, tol: float) -> float:
    """Score peaks at `ideal`, falls off linearly to 0 at `ideal +/- tol`."""
    if tol <= 0:
...
def _min_score(actual: float, floor: float, tol: float) -> float:
    """Score is 1.0 once `actual` reaches `floor`; falls off below it."""
    if actual >= floor:
        return 1.0
    if tol <= 0:
        return 0.0
...
def _max_score(actual: float, ceiling: float, tol: float) -> float:
    """Score is 1.0 while `actual` stays under `ceiling`; falls off above it."""
    if actual <= ceiling:
        return 1.0
    if tol <= 0:
        return 0.0
...
def _weighted(parts: list) -> float:
    """parts: list of (score_0_to_1, weight). Returns weighted 0..1 total."""
...
def _quality_ok(lm, pose_name):
    ...
    supine_keys = [11,12,23,24,25,26,27,28]
    ...
    elif pose_name == "setu_bandhasana":
        keys, min_vis, coverage = supine_keys, 0.15, 0.40
...
def _strict_shape_ok(pose_name, lm):
    ...
    if pose_name == "setu_bandhasana":
       
        hips_raised = hiy < shy + 0.08
        knees_below = _avg_y(lKn, rKn) > hiy - 0.05
        bridge_height_ok = (_avg_y(lKn, rKn) - hiy) >= 0.02
        body_present = _body_height(lm) > 0.18
        conditions = [hips_raised, knees_below, bridge_height_ok, body_present]
...
VALID_POSES = {"tree", "warrior", "padmasana", "vajrasana", "baddha_konasana", "tadasana", "trikonasana", "setu_bandhasana", "bhujangasana", "wall_plank_chaturanga", "padahastasana", "paschimottanasana", "paschim_namaskarasana", "pranayama"}

HOLD_POSES = {"padmasana", "baddha_konasana", "vajrasana", "tadasana", "setu_bandhasana", "bhujangasana", "wall_plank_chaturanga", "padahastasana", "paschimottanasana", "paschim_namaskarasana", "pranayama", "warrior"}
...
POSE_PASS_THRESHOLDS = {
    ...
    "setu_bandhasana": 40, "bhujangasana": 65, "wall_plank_chaturanga": 65,
...
POSE_TOLERANCES = {
    ...
    "setu_bandhasana": {
        ...
        "knee_ideal": 110,         "knee_tol": 90,         "knee_w": 0.15,
        "hip_lift_ideal": 0.08,    "hip_lift_tol": 0.22,   "hip_lift_w": 0.35,
        "shoulder_ground_ideal": 0.0, "shoulder_ground_tol": 0.22, "shoulder_ground_w": 0.20,
        "feet_width_ceiling": 0.24, "feet_width_tol": 0.28, "feet_width_w": 0.30,
        "knee_bent_max": 175, "hip_lift_min": 0.02, "feet_width_max": 0.40,
    },
    "bhujangasana": {
...
class PoseChecker:
    ...
    def score(self, pose_name: str, landmarks: list) -> float:
        ...
        fn = {
            ...
            "setu_bandhasana": self._score_setu_bandhasana,
    ...
    def validate(self, pose_name: str, landmarks: list) -> dict:
        ...
        dispatch = {
            ...
            "setu_bandhasana": self._validate_setu_bandhasana,
    ...
    def _score_setu_bandhasana(self, lm: list) -> float:
        t = POSE_TOLERANCES["setu_bandhasana"]
        l_sh, r_sh = lm[11], lm[12]
        l_hi, r_hi = lm[23], lm[24]
        l_kn, r_kn = lm[25], lm[26]
        l_an, r_an = lm[27], lm[28]
        ...
        knee_y = (l_kn["y"] + r_kn["y"]) / 2

        hip_lift_score = _min_score(knee_y - hip_y, t["hip_lift_ideal"], t["hip_lift_tol"])
        min_knee_ang = min(angle_between(l_hi, l_kn, l_an), angle_between(r_hi, r_kn, r_an))
        knee_score = _target_score(min_knee_ang, t["knee_ideal"], t["knee_tol"])
        shoulder_ground_score = _max_score(shoulder_y - hip_y, t["shoulder_ground_ideal"], t["shoulder_ground_tol"])
        feet_score = _max_score(lateral_distance(l_an, r_an), t["feet_width_ceiling"], t["feet_width_tol"])

        return _weighted([
            (hip_lift_score, t["hip_lift_w"]),
            (knee_score, t["knee_w"]),
            (shoulder_ground_score, t["shoulder_ground_w"]),
            (feet_score, t["feet_width_w"]),
        ])

    def _validate_setu_bandhasana(self, lm, score):
        if len(lm) < REQUIRED_LANDMARKS:
            return [], "No pose detected - stand where your shoulders, hips, and knees are all visible."
        t = POSE_TOLERANCES["setu_bandhasana"]
        ...
        hip_y = (l_hi["y"] + r_hi["y"]) / 2
        knee_y = (l_kn["y"] + r_kn["y"]) / 2
        hips_up = (knee_y - hip_y) >= t["hip_lift_min"]
        knees_bent = min(angle_between(l_hi, l_kn, l_an), angle_between(r_hi, r_kn, r_an)) <= t["knee_bent_max"]
        feet_ok = lateral_distance(l_an, r_an) <= t["feet_width_max"]

        criteria = [
            {"name": "Hips lifted", "passed": hips_up, "hint": "Lift your hips higher."},
            {"name": "Knees bent", "passed": knees_bent, "hint": "Keep knees bent with feet near hips."},
            {"name": "Feet stable", "passed": feet_ok, "hint": "Ground both feet evenly."},
        ]

        if not hips_up: hint = "Lift your hips to rebuild the bridge."
        elif not knees_bent: hint = "Bend knees more and keep feet under knees."
        elif not feet_ok: hint = "Keep feet grounded and controlled."
        else: hint = "Strong Bridge Pose - the kingdom routes are reconnecting."
        return criteria, hint
