(function () {
  ...
  const KNOWN_WORLD_POSES = new Set([
    ...
    "trikonasana", "setu_bandhasana", "bhujangasana", "wall_plank_chaturanga",
  ...
  function renderJungleTemple(W, H) {
    ...
    // sky from uploaded snake garden animation
    ...
    // animated snake (cobra / Bhujangasana inspired)
  ...
  function renderSamuraiDojo(W, H) {
    ...
    // table and tea set (sits on tatami)
  ...
  const GV = (() => {
    ...
    //  RNG seeded for deterministic ground leaves 
    ...
    function rebuildLeafCarpet() {
      ...
      for (let i = 0; i < count; i++) {
        const x = rng() * W;
        const depthT = rng(); // 0 = near horizon, 1 = near bottom
        const y = groundY + depthT * depthT * (H - groundY);
        const scale = 0.5 + depthT * 0.9; // bigger closer
        const hue = 10 + rng() * 35; // red-orange-amber
        const sat = 65 + rng() * 30;
        const lit = 30 + rng() * 28;
    ...
    function drawTree(tx, groundY, trunkH, trunkW, leafDensity, hue, windT) {
      ...
      // Extra sweeping branch arching outward
    ...
    function drawGround(blend) {
      ...
      if (carpetAlpha > 0.01) {
        ...
        carpetLeaves.forEach(leaf => {
          // Near horizon leaves are more transparent (depth fade)
    ...
    function render(blend) {
      ...
      // Left massive tree (like the big maple in the photo)
      const fgTrees = [
      ...
      // Falling leaves (few, natural drift)
  ...
  const LEVEL_NAMES = {
    ...
    setu_bandhasana: ["Reconnect the Village", "Restore the Trading Route", "Unite the Settlements", "Kingdom Alliance Restored"],
  ...
  function loop() {
    ...
    switch (gPose) {
      ...
      case "setu_bandhasana": renderKingdoms(W, H); break;
