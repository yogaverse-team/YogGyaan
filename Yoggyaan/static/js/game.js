const HOLD_POSE_NAMES = new Set(["tree", "padmasana", "baddha_konasana", "vajrasana", "tadasana", "setu_bandhasana", "bhujangasana", "wall_plank_chaturanga", "padahastasana", "paschimottanasana", "paschim_namaskarasana", "pranayama", "warrior"]);
...
const LEVEL_MILESTONES = {
  ...
  // Generic placeholder used only when a pose has no registered milestones.
  // Must stay pose-neutral so unregistered poses never inherit Tree Pose copy.
...
window.startPractice = async function () {
  ...
  if (isHoldPose) {
    // Reps are only ever asked once, before the tutorial (see askRepsBeforeTutorial
    // in game.html). Never prompt again here — fall back to a sane default only if
    // that earlier step somehow didn't run.
