(function () {
  ...
  window.STEP_TITLES = {
    ...
    setu_bandhasana: ["Lie and Set Feet", "Bend Knees", "Lift Bridge", "Hold Strong"],
  };
...
const POSE_TOLERANCES_JS = {
  ...
  setu_bandhasana: {
    kneeIdeal: 90, kneeTol: 40, kneeW: 0.30,
    hipLiftIdeal: 0.14, hipLiftTol: 0.14, hipLiftW: 0.35,
    shoulderGroundCeiling: 0.0, shoulderGroundTol: 0.14, shoulderGroundW: 0.15,
    feetWidthCeiling: 0.18, feetWidthTol: 0.20, feetWidthW: 0.20,
  },
  bhujangasana: {
...
function qualityGate(lm, poseName) {
  ...
  const supineKeys = [11, 12, 23, 24, 25, 26, 27, 28];
  ...
  } else if (poseName === "setu_bandhasana") {
    required = supineKeys; minVis = 0.20; coverage = 0.50; minBoxW = 0.08; minBoxH = 0.10;
...
function computeAccuracy(lm, poseName) {
  ...
  if (poseName === "tadasana") {
    ...
    const hipsRaised = hiY < shY + 0.03;
    const kneesBelow = knY > hiY + 0.02;
    const bridgeHeightOk = (knY - hiY) >= 0.05;
    const passCount = [hipsRaised, kneesBelow, bridgeHeightOk].filter(Boolean).length;
    if (passCount < 2) {
      setPoseDebug(poseName, 0, {}, "Lie down and lift hips for Bridge; sitting cannot count");
      return 0;
    }
    score = weighted([
      [targetScore(Math.min(lKnAng, rKnAng), t.kneeIdeal, t.kneeTol), t.kneeW],
      [minScore(knY - hiY, t.hipLiftIdeal, t.hipLiftTol), t.hipLiftW],
      [maxScore(shY - hiY, t.shoulderGroundCeiling, t.shoulderGroundTol), t.shoulderGroundW],
      [maxScore(Math.abs(lAn.x - rAn.x), t.feetWidthCeiling, t.feetWidthTol), t.feetWidthW],
    ]);
  }
